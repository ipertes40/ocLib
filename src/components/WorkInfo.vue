...
export default {
  ...
  data() {
    return {
      imgBasepath: "",
      // 包含的全部oc
      ocs: qs.parse(this.relateOc).relateOc,
      worlds: qs.parse(this.relateWorld).relateWorld,
    };
  },
  created() {
    // 获取基础路径
  ...
  methods: {
    toWorld(id) {
      this.$router.push("/detailPage/world/" + id);
    },
    toOc(id) {
      this.$router.push("/detailPage/oc/" + id);
    },
    editmsg() {
